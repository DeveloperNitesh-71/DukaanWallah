...
const Product = () => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-32 space-y-4">
                ...
                <p className="text-gray-500 font-bold animate-pulse uppercase tracking-[0.2em] text-xs">Loading Fresh Products...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="text-center py-32 bg-red-50 rounded-[40px] border-2 border-dashed border-red-200 max-w-7xl mx-auto px-4 mt-8">
                <div className="text-6xl mb-6">⚠️</div>
                <h4 className="text-2xl font-black text-gray-900 mb-2">Error Loading Products</h4>
                <p className="text-red-500 max-w-xs mx-auto mb-6">{error}</p>
                <button 
                    onClick={() => window.location.reload()}
                    className="text-green-600 font-black hover:underline underline-offset-8"
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-12 pb-24'>
                {filteredProducts.map((product, index) => (
                    <div key={product._id} className="animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both" style={{animationDelay: `${index * 100}ms`}}>
