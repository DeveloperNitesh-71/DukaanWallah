...
const Dashboard = () => {
  ...
  const overviewFilteredOrders = useMemo(() => {
    ...
    const last7Days = new Date(Date.now() - 7 * 86400000).setHours(0,0,0,0);
    const last30Days = new Date(Date.now() - 30 * 86400000).setHours(0,0,0,0);
    ...
    return orders.filter(order => {
      ...
      if (dateFilter === 'Today') {
        ...
        const customDate = new Date(selectedDate).setHours(0,0,0,0);
  ...
  const todaysPendingOrders = useMemo(() => {
    ...
    return orders.filter(order => {
      const orderDate = new Date(order.createdAt).setHours(0,0,0,0);
      return orderDate === today && (order.status.toLowerCase() === 'pending' || order.status.toLowerCase() === 'processing');
    });
  }, [orders]);
