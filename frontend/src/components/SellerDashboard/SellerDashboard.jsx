          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:block text-right">
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest leading-none mb-1">{userInfo?.role === 'seller' ? 'Seller' : 'Admin'}</p>
                <p className="text-sm font-black text-gray-900 leading-none">{userInfo?.name || 'Guest User'}</p>
              </div>
              <div className="h-10 w-10 bg-gradient-to-br from-green-100 to-green-50 rounded-xl border border-green-200 flex items-center justify-center font-black text-green-700 shadow-sm">
                {userInfo?.name?.split(' ').map(n => n[0]).join('') || 'GU'}
              </div>
            </div>
          </div>
        </header>
