...
import { IoMdPersonAdd, IoMdSearch, IoMdMore, IoMdCall, IoMdPin } from 'react-icons/io';
import AddCustomerModal from './AddCustomerModal';

const Customers = () => {
  const { customers, addCustomer, maxCustomers } = useCustomers();
                ...
                filteredCustomers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gray-900 text-white flex items-center justify-center font-black text-lg shadow-lg group-hover:bg-green-600 transition-colors">
                          {customer.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-black text-gray-900 text-sm">{customer.shopName}</p>
                          <p className="text-[10px] font-black text-gray-400 mt-1 uppercase tracking-widest">ID: {customer.id} • {customer.name}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <span className="text-xs font-black text-gray-900 bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                        {new Date(customer.joinedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </span>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <button className="p-2.5 bg-gray-50 text-gray-400 hover:bg-white hover:text-gray-900 rounded-xl transition-all active:scale-95 shadow-sm border border-transparent hover:border-gray-100">
