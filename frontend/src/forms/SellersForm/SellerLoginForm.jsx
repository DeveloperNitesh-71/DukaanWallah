  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (user.role === 'seller') {
        ...
        showToast('Access denied. This is a seller login.', 'error');
      }
    } catch (err) {
      console.error(err);
    }
    ...
    <div className='relative w-full max-w-[440px] p-10 border border-white/10 bg-white/5 rounded-3xl backdrop-blur-2xl shadow-2xl flex flex-col gap-8'>
      ...
      <form className='flex flex-col gap-4' onSubmit={submitHandler}>
        ...
        <button 
          type="submit"
          disabled={loading}
          className='w-full py-5 bg-orange-500 text-black font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-orange-400 transition-all transform active:scale-[0.98] shadow-xl shadow-orange-900/20 mt-4 disabled:opacity-50'
        >
